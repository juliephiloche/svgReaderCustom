...
define(["qlik","jquery", "./d3", "./chroma", "core.utils/theme", "./svgOptions", "./svgFunctions", "./senseUtils"], function (qlik, $, d3, chroma, Theme) {
	...
	//TODO load svg and get children array	
	var customOptions;
	var customdisplaycheck = false;
	var svgChildrenArray;
	...
	customOptions = function(){
		...
		// first part of custom options - fixed
		jsonStr =  '{										';
		jsonStr += '	"label": "Custom Visual",             ';
		jsonStr += '	"type":"items",                       ';
		jsonStr += '	"items":{                             ';
		jsonStr += '		"manageElem": {                   ';
		jsonStr += '			"label": "Manage Element",    ';
		jsonStr += '			"type": "boolean",            ';
		jsonStr += '			"component": "switch",        ';
		jsonStr += '			"options": [                  ';
		jsonStr += '				{                       ';
		jsonStr += '					"label": "Off",       ';
		jsonStr += '					"value": false        ';
		...
		jsonStr += '					"value": true         ';
		jsonStr += '				}],                     ';
		jsonStr += '			"ref": "customdisplay",       ';
		jsonStr += '			"defaultValue": false         ';
		jsonStr += '		},                              ';
		jsonStr += '		"manageElemName":{                ';
		jsonStr += '			"type": "string",             ';
		jsonStr += '			"label": "Element ID",        ';
		jsonStr += '			"ref": "customparent",        ';
		jsonStr += '			"expression": "optional"     ';
		jsonStr += '		}';
		
		// second part of custom options - dynamic
		
		console.log(svgChildrenArray);									// DEBUG
		if(svgChildrenArray){
			
			jsonStr += ',';
			
			//for(var i=0; i<svgChildrenArray.length; i++){
			svgChildrenArray.foreach(function(index){
				
				console.log(i+" : "+svgChildrenArray[index]);			// DEBUG
				
				var arr = svgChildrenArray[index].split("@");
				var id = arr[0];
				var val = arr[1];
				
				if(index>0) jsonStr += ',';	// all except the first one
				
				jsonStr += '		"'+id+'": {';
				jsonStr += '			"type": "boolean",';
				jsonStr += '			"label": "'+id+'",';
				jsonStr += '			"ref": "customdisplay.'+id+'"';
				jsonStr += '		}';
			});
		}
		
		// third part of custom options - end
	...
	return {
		...
		paint: function ($element, layout, self) {
			...
			customdisplaycheck = layout.customdisplay;
					...
					if (loadThis == "NO VARIABLE") {
						...
						d3.xml(loadThis, "image/svg+xml", function (xml) { //load the SVG
							if (xml) { //if it loaded properly...
								...
								// OLD STEP, GET THE CHILDREN LIST
								
								// want a custom display, with a defined and existing parent
								if(layout.customdisplay && layout.customparent && $("#" +layout.customparent).length){
									//console.log($("#" +layout.customparent).length);
									//console.log($("#" +layout.customparent)[0].children);
									//console.log($("#" +layout.customparent)[0].children.length)
									
									var lg = $("#" +layout.customparent)[0].children.length;
									
									//console.log(svgChildrenArray);
									
									for(var cpt=0; cpt<lg; cpt++) {
										//console.log($("#"  +layout.customparent)[0].children[cpt]);
										var id = $("#"  +layout.customparent)[0].children[cpt].id;
										
										//console.log("id : "+id);
										
										if(svgChildrenArray==undefined)
											svgChildrenArray = [];
										if(svgChildrenArray[id] == undefined)
											svgChildrenArray[id] = id+"@"+true;
